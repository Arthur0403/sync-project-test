-- TODO: в целом работает, но на будущее нужно
-- будет переписать открытие закрытие терминалов на более удобный вариант.
-- Также не понятно как это будет работать с чистым nvim. Пока рассчитано только на
-- AstroNvim
local M = {
	FILENAME = "config.ini",
	DIRNAME = ".nvim_config",
